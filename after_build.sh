...
export varzip=plr-${gitrevshort}-pg${pgversion}-R${rversion}${rversion_more}-${Platform}-${Configuration}-${compiler}.zip
loginfo "${varzip}"

echo ${APPVEYOR_BUILD_FOLDER}

loginfo "BEGIN plr zip CREATION"
7z a -r                   ${APPVEYOR_BUILD_FOLDER}/${varzip} ./tmp/*
ls -alrt                  ${APPVEYOR_BUILD_FOLDER}/${varzip}
loginfo "BEGIN plr zip LISTING"
7z l                      ${APPVEYOR_BUILD_FOLDER}/${varzip}
...
then
  ...
  loginfo "appveyor PushArtifact ${varzip}"
           appveyor PushArtifact ${varzip}
  popd
  #
  # BAD PUSH-ARTIFACT - DEFINITELY A BUG
  #
  # loginfo "appveyor PushArtifact ${APPVEYOR_BUILD_FOLDER}/${varzip}"
  #          appveyor PushArtifact ${APPVEYOR_BUILD_FOLDER}/${varzip}
  #
